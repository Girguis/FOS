            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        {
                            String imgUrl=data.select("div.gallery-item").select("img").eq(i).attr("src");
                            String title=data.select("div.gallery-item").select("div.gallery-desc").select("h3").select("a").eq(i).text();
                            String detailsUrl=data.select("div.gallery-item").select("div.gallery-desc").select("h3").select("a").eq(i).attr("href");
